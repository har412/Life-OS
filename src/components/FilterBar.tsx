...
function DD<T extends string>({ label, options, selected, onToggle, onClear, renderOpt }: {
  label: string; options: T[]; selected: T[]; onToggle:(v:T)=>void; onClear:()=>void; renderOpt:(v:T)=>React.ReactNode;
}) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white border border-stone-200 rounded-xl shadow-lg min-w-[160px] py-1.5">
          {options.map(o => (
      ...
      {mobileOpen && (
        <div className="lg:hidden mt-3 p-3 bg-stone-50 border border-stone-200 rounded-xl space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <DateFilter/>
            <DD label="Category" options={allCategories.map(c=>c.id)} selected={filters.categories} onToggle={v=>updateFilter("categories",toggle(filters.categories,v))} onClear={()=>updateFilter("categories",[])} renderOpt={c=>{const m=getCatMeta(c,allCategories);return <span className="flex items-center gap-2 text-sm text-stone-700"><span className={`w-2 h-2 rounded-full ${m.dot}`}/>{m.label}</span>;}}/>
            <DD label="Priority" options={ALL_PRIS} selected={filters.priorities} onToggle={v=>updateFilter("priorities",toggle(filters.priorities,v))} onClear={()=>updateFilter("priorities",[])} renderOpt={p=><span className="flex items-center gap-2 text-sm text-stone-700"><span className={`w-2 h-2 rounded-full ${PRIORITY_META[p].dot}`}/>{PRIORITY_META[p].label}</span>}/>
            <DD label="Status" options={ALL_STATS} selected={filters.statuses} onToggle={v=>updateFilter("statuses",toggle(filters.statuses,v))} onClear={()=>updateFilter("statuses",[])} renderOpt={s=><span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_META[s].badge} ${STATUS_META[s].text}`}>{STATUS_META[s].label}</span>}/>
          </div>
          <div className="flex gap-2">
